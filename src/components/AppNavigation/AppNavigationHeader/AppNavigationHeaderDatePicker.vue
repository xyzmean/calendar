...
// The format has to be a new function whenever the language or the view
// changes: the picker only re-renders its label when the prop changes, so a
// wrapper that reads them at call time kept the old period header.
...
const dateFormatFn = computed<NcDateTimePickerFormatProp>(() => {
	const momentLocaleValue = momentLocale.value
